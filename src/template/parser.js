...
const parseOutgoingBody = (ctx, outputFormat) => {
  ...
  if (outputFormat === 'XML') {
    try {
      ...
      throw new Error(
        `Template Transform: Parsing outgoing body failed: ${error.message}`
      )
    }
  }
  ...
  logger.info(
    `Template Transform: Parsing outgoing body into ${outputFormat} format`
  )
}
...
const parseIncomingBody = async (ctx, inputFormat) => {
  ...
  if (ALLOWED_CONTENT_TYPES.includes(inputFormat)) {
    ...
    logger.info(
      `Template Transform: Parsing incoming body into JSON format for processing`
    )

    try {
      if (!ctx.request.body.body) {
        throw Error(
          `Template Transform: Parsing incoming body not found`
        )
      }
      ...
      await KoaBodyParser(options)(ctx, () => {
        ...
        ctx.state.allData.requestBody = ctx.request.body.body
      ...
      throw Error(
        `Template Transform: Parsing incoming body failed: ${error.message}`
      )
    }
  } else {
    throw new Error(
      `Template Transform: transformation method "${inputFormat}" not yet supported`
    )
  }
}
...
exports.parseBodyMiddleware = () => async (ctx, next) => {
  ...
  const inputContentType = contentType.includes('json') ? 'JSON' : contentType.includes('xml') ? 'XML' : contentType
  const outputContentType = inputContentType
